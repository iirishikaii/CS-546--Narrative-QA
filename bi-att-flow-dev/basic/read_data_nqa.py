...
def get_squad_data_filter(config):
    def data_filter(data_point, shared):
        #for each data point it checks various conditions, and if they are not true returns false
        #for speedup
        assert shared is not None
        rx, rcx, q, cq = (data_point[key] for key in ('*x', '*cx', 'q', 'cq'))
...
def update_config(config, data_sets):
    ...
    for data_set in data_sets:
        ...
        for idx in data_set.valid_idxs:
            rx = data['*x'][idx]
            q = data['q'][idx]
            sents = shared['x'][rx[0]][0] #refers each summary
    ...
    print ("max_para_size",config.max_para_size,"max_num_sents",config.max_num_sents,
           "max_sent_size",config.max_sent_size,"max_word_size",config.max_word_size,"max_ques_size",config.max_ques_size)
